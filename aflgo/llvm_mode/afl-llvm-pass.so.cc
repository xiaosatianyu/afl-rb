...
cl::opt<std::string> HitTargets("hittargets",
		cl::desc(
				"the targets to check if they are hit."),
		cl::value_desc("hittargets"));
...
bool AFLCoverage::runOnModule(Module &M) {  //这里是将整个系统都当做一个类了吧
	...
	if (!TargetsFile.empty()) {
		...
		if (!HitTargets.empty()){
			std::string line_hit;
			std::ifstream hittargetsfile(HitTargets);
			while (std::getline(hittargetsfile, line_hit))
				hittargets.push_back(line_hit);
			hittargetsfile.close();
		}
	}
					...
					for (auto &I : BB) {
						...
						if (DILocation *Loc = I.getDebugLoc()) {
							...
							if ( find(hittargets.begin(), hittargets.end(), inst_name) != hittargets.end()){
								...
								LoadInst *MapHit = IRB.CreateLoad(LargestType, MapHitPtr); //读取MapDistPtr位置的数据
								MapHit->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));

								//add the hit number
								Value *IncrHit = IRB.CreateAdd(MapHit, ConstantInt::get(LargestType, 1)); //击中就加1
								IRB.CreateStore(IncrHit, MapHitPtr)->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None)); //保存
							}
							//end check
						}
