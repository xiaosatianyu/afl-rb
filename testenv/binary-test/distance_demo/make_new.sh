...
export CC=$AFLGO/afl-clang-fast
export CXX=$AFLGO/afl-clang-fast++
export CFLAGS="$COPY_CFLAGS -distance=$TMP_DIR/distance.cfg.txt  -hittargets=$TMP_DIR/BBtargets.txt"
export CXXFLAGS="$COPY_CXXFLAGS -distance=$TMP_DIR/distance.cfg.txt  -hittargets=$TMP_DIR/BBtargets.txt"

# 重新生成
cd $SUBJECT
make clean
AFL_DONT_OPTIMIZE=1   make -j8
cp $SUBJECT/test $CurDir/test-no-asan

make clean
AFL_DONT_OPTIMIZE=1  AFL_USE_ASAN=1 make -j8
cp $SUBJECT/test $CurDir/test-asan
